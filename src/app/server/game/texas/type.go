...
//计算德州扑克的牌型(返回最大5张牌)
func CardTypeOfTexas(list []int16) (int, []int16) {
	this := new(card_calculator)
	this.point = 0
	this.big_list = make([]int16, 5)
...
func (this *card_calculator) GetType() int {
	if ret := this.straight_flush(); ret != 0 {
		return ret
...
func (this *card_calculator) PushTypeCard(card *CardData) {
	this.big_list[this.point] = card.num
	this.point++
}
...
func (this *card_calculator) SupplyCards() []int16 {
	if this.point < POCKET_CARD_SIZE {
		//补全剩余的牌
...
func CompareCards(lsa []int16, lsb []int16, size int) int {
	for i := 0; i < size; i++ {
		if CardVal(lsa[i]) > CardVal(lsb[i]) {
			return 1
		} else if CardVal(lsa[i]) < CardVal(lsb[i]) {
			return -1
		}
	}
	return 0
